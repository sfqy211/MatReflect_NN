function [colors] = colorOpt(rho, lobes, x0, metric)
    ...
    imageRgb = renderSlice * rho;
    imageHsiWrap = wrapHsi(rgb2hsi(imageRgb));
...
%         funVal = sum(sum((imageHsiWrap(:, 1:2) - hsiWrapRecon(:, 1:2)) .^ 2)) + sum((imageHsiWrap(:, 2) .* diffAngle(imageHsiWrap(:, 3) - hsiWrapRecon(:, 3))) .^ 2);
    ...
    function [funVal] = image2rgb(x)
        rgbRecon = lobeImages * reshape(x, channelNum, lobeNum)';
%         funVal = sum(sum((imageHsiWrap(:, 1:2) - hsiWrapRecon(:, 1:2)) .^ 2)) + sum((imageHsiWrap(:, 2) .* diffAngle(imageHsiWrap(:, 3) - hsiWrapRecon(:, 3))) .^ 2);
        funVal = sum((rgbRecon(:) - imageRgb(:)) .^ 2);
    end
