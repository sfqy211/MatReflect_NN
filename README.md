...
**MatReflect_NN** 是一个集成传统物理渲染（Mitsuba）与现代深度学习技术（Neural-BRDF / HyperBRDF）的研究平台。本项目当前同时保留：
...
| **🧠 Model Training** | **Neural-BRDF**: 基于 MLP 的单材质过拟合训练 (PyTorch/Keras)。<br>**HyperBRDF** / **DecoupledHyperBRDF**: 基于 HyperNetwork 的多材质泛化重建。 | `nbrdf-train` (Neural-BRDF)<br>`hyperbrdf` (HyperBRDF)<br>`decoupledhyperbrdf` (Decoupled) |
...
### 5. 训练环境 C: `decoupledhyperbrdf` (DecoupledHyperBRDF)

`DecoupledHyperBRDF` 必须使用独立环境，不与 `hyperbrdf` 共享 checkpoint 资产。环境依赖可先基于 `hyperbrdf` 克隆，再按需要继续调整：

```bash
conda create --name decoupledhyperbrdf --clone hyperbrdf -y
```

注意：

- `HyperBRDF` 的 checkpoint / `.pt` 不能默认当作 `DecoupledHyperBRDF` 的 checkpoint 使用。
- 两者训练结果和提取参数应分别管理。
...
3.  **训练神经材质**:
    ...
    *   **HyperBRDF / DecoupledHyperBRDF**: 训练或选择 checkpoint -> 点击“启动参数提取” -> 生成 `.pt` 参数 -> 点击“执行重建转换” -> 生成 `.fullbin`。
