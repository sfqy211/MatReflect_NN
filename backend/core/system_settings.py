...
def build_default_system_settings() -> SystemSettings:
    ...
    return SystemSettings(
        ...
        materials_dir=str((data_root / "materials").resolve()),
        nbrdf_render_input_dir=str((data_root / "render-input" / "neural-brdf").resolve()),
        hyperbrdf_render_input_dir=str((data_root / "render-input" / "hyperbrdf").resolve()),
        merl_render_dir=str((data_root / "renders" / "merl").resolve()),
        nbrdf_render_dir=str((data_root / "renders" / "neural-brdf").resolve()),
        hyperbrdf_render_dir=str((data_root / "renders" / "hyperbrdf").resolve()),
        snbrdf_render_dir=str((data_root / "renders" / "hypersnbrdf").resolve()),
        grids_dir=str((data_root / "analysis" / "grids").resolve()),
        comparisons_dir=str((data_root / "analysis" / "comparisons").resolve()),
...
def _coerce_settings(raw_data: dict) -> SystemSettings:
    ...
    return SystemSettings(
        ...
        materials_dir=str(raw_data.get("materials_dir") or defaults.materials_dir),
        nbrdf_render_input_dir=str(raw_data.get("nbrdf_render_input_dir") or defaults.nbrdf_render_input_dir),
        hyperbrdf_render_input_dir=str(raw_data.get("hyperbrdf_render_input_dir") or defaults.hyperbrdf_render_input_dir),
        merl_render_dir=str(raw_data.get("merl_render_dir") or defaults.merl_render_dir),
        nbrdf_render_dir=str(raw_data.get("nbrdf_render_dir") or defaults.nbrdf_render_dir),
        hyperbrdf_render_dir=str(raw_data.get("hyperbrdf_render_dir") or defaults.hyperbrdf_render_dir),
        snbrdf_render_dir=str(raw_data.get("snbrdf_render_dir") or defaults.snbrdf_render_dir),
        grids_dir=str(raw_data.get("grids_dir") or defaults.grids_dir),
        comparisons_dir=str(raw_data.get("comparisons_dir") or defaults.comparisons_dir),
